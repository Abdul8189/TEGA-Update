  ...
  try {
    ...
    do {
      ...
      if (attempts >= maxAttempts) {
        ...
        console.warn(`Could not generate unique random ID after ${maxAttempts} attempts, using timestamp: ${studentId}`);
        break;
      }
...
studentSchema.pre('validate', async function(next) {
  ...
  if (this.isNew && !this.studentId) {
    try {
      ...
      this.isAutoGeneratedId = true;
      console.warn(`Using fallback student ID: ${this.studentId}`);
    }
  }
  next();
});
