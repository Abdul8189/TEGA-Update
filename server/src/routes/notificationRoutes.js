...
router.get('/admin/payments', adminAuth, async (req, res) => {
  try {
    ...
    console.error('Get admin payment notifications error:', error);
  ...
  try {
    ...
    if (isMongoConnected()) {
      try {
        ...
        console.log('MongoDB notifications query failed, using in-memory storage');
      }
    }
    ...
    if (notifications.length === 0 && userNotifications.has(userId)) {
      ...
      console.log(`Retrieved ${notifications.length} notifications from in-memory storage for user ${userId}`);
    }
  ...
  try {
    ...
    if (isMongoConnected()) {
      try {
        ...
        console.log('MongoDB mark as read failed, using in-memory storage');
      }
    }
    ...
    if (!notification && userNotifications.has(userId)) {
      ...
      if (notifIndex !== -1) {
        ...
        console.log(`Marked notification as read in in-memory storage for user ${userId}`);
      }
    }
  ...
  try {
    ...
    if (isMongoConnected()) {
      try {
        ...
        console.log('MongoDB delete notification failed, using in-memory storage');
      }
    }
    ...
    if (!notification && userNotifications.has(userId)) {
      ...
      if (notifIndex !== -1) {
        ...
        console.log(`Deleted notification from in-memory storage for user ${userId}`);
      }
    }
