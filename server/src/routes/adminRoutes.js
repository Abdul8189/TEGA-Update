...
router.get('/principals/test/:principalId', adminAuth, async (req, res) => {
  try {
    const { principalId } = req.params;
    console.log('Testing principal lookup with ID:', principalId);
    
    const principal = await Principal.findById(principalId);
    console.log('Principal found:', principal ? 'Yes' : 'No');
...
router.post('/bulk-import', adminAuth, async (req, res) => {
  try {
    ...
    console.error('Error saving students:', saveError);
    
    // Log detailed error information
    if (saveError.name === 'ValidationError') {
      console.error('Validation error details:', {
        message: saveError.message,
        errors: saveError.errors ? Object.keys(saveError.errors) : 'No error details',
...
router.delete('/principals/:principalId', adminAuth, async (req, res) => {
  ...
  try {
    ...
    console.log('Attempting to delete principal with ID:', principalId);
    
    // First, let's check if the principal exists
    const existingPrincipal = await Principal.findById(principalId);
    if (!existingPrincipal) {
      console.log('Principal not found with ID:', principalId);
      return res.status(404).json({ success: false, message: 'Principal not found' });
    }
    
    console.log('Found principal to delete:', existingPrincipal.principalName);
    
    // Now try to delete
    const principal = await Principal.findByIdAndDelete(principalId);
    
    if (!principal) {
      console.log('Principal deletion failed');
      return res.status(500).json({ success: false, message: 'Principal deletion failed' });
    }

    console.log('Successfully deleted principal:', principal.principalName);
...
router.put('/users/:userId', adminAuth, async (req, res) => {
  try {
    ...
    console.log('  - Gender value from request:', updatedData.gender, '(type:', typeof updatedData.gender, ')');
    ...
    if (cleanedData.gender) {
      console.log('🔍 Original gender value:', cleanedData.gender, '(type:', typeof cleanedData.gender, ')');
      const normalizedGender = cleanedData.gender.toLowerCase().trim();
      console.log('🔍 Normalized gender:', normalizedGender, '(type:', typeof normalizedGender, ')');
      
      // Validate against enum values
      const validGenders = ['male', 'female', 'other'];
      if (!validGenders.includes(normalizedGender)) {
        console.error('❌ Invalid gender value:', normalizedGender, 'Valid values:', validGenders);
      ...
      console.log('🔍 Final gender value for database:', cleanedData.gender);
    }
    ...
    console.log('🔍 Attempting to update as student...');
    console.log('🔍 Student update data:', cleanedData);
    console.log('🔍 Student update gender:', cleanedData.gender, '(type:', typeof cleanedData.gender, ')');
    ...
    // If not found as student, try as principal
    if (!user) {
      console.log('🔍 Student not found, attempting to update as principal...');
      try {
        ...
        console.error('❌ Principal update error:', principalError.message);
        console.error('❌ Principal validation errors:', principalError.errors);
        console.error('❌ Principal error details:', principalError);
    ...
    console.error('❌ Update user error message:', error.message);
    console.error('❌ Update user error stack:', error.stack);
    console.error('❌ Update user request body:', req.body);
    console.error('❌ Update user request params:', req.params);
    res.status(500).json({ 
      success: false, 
...
router.get('/students/college/:collegeName', adminAuth, async (req, res) => {
  try {
    ...
    console.log('🎯 Admin: College students route called');
    console.log('🎯 College name from params:', collegeName);
    console.log('🎯 Decoded college name:', decodeURIComponent(collegeName));
    ...
    console.log('🎯 Admin: Fetching students for college:', decodedCollegeName);
    ...
    console.log('🎯 Sample institutes:', allStudents.slice(0, 5).map(s => s.institute));
    ...
    console.log(`🎯 Admin: Found ${students.length} students for college: ${decodedCollegeName}`);
    
    // Log student details for debugging
    if (students.length > 0) {
      console.log('🎯 Admin: Student details:', students.map(s => ({
...
router.post('/students/bulk-import', adminAuth, async (req, res) => {
  try {
    ...
    for (const [index, student] of students.entries()) {
      ...
      try {
        ...
        console.error(`Error processing student at row ${rowNumber}:`, studentError);
  ...
  if (studentsToInsert.length === 0) {
    ...
    console.log(`Attempting to insert ${studentsToInsert.length} students`);
    
    try {
      console.log('Starting to save students to database...');
...
router.post('/register-principal', adminAuth, async (req, res) => {
  try {
      ...
      const mailOptions = {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #7c3aed;">Welcome, ${newPrincipal.principalName}!</h2>
            <p>An admin has created a principal account for you on the TEGA Platform.</p>
            <p>You can now log in using the following credentials:</p>
            <ul>
              <li><strong>Email:</strong> ${newPrincipal.email}</li>
    ...
    console.error('Register principal error:', error);
    res.status(500).json({
      success: false,
...
router.get('/payment-notifications', adminAuth, async (req, res) => {
  try {
    console.log('📊 Admin: Fetching payment notifications...');
    ...
    console.log('📊 Admin: Found', paymentNotifications.length, 'payment notifications');

    res.json({ success: true, data: paymentNotifications });
  } catch (error) {
    console.error('❌ Admin: Get payment notifications error:', error);
...
router.get('/payments', adminAuth, async (req, res) => {
  try {
    ...
    console.log('📊 Admin: Found', oldPayments.length, 'old payments and', razorpayPayments.length, 'Razorpay payments');
    
    // Debug: Check if we have any payments
    if (oldPayments.length > 0) {
      console.log('📊 Admin: Sample old payment:', {
        id: oldPayments[0]._id,
        paymentMethod: oldPayments[0].paymentMethod,
        status: oldPayments[0].status
      });
    }
    
    if (razorpayPayments.length > 0) {
      console.log('📊 Admin: Sample Razorpay payment:', {
        id: razorpayPayments[0]._id,
        status: razorpayPayments[0].status
      });
    }
    ...
    console.log('📊 Admin: Normalized old payments sample:', normalizedOldPayments[0] ? {
      id: normalizedOldPayments[0]._id,
      source: normalizedOldPayments[0].source,
      paymentMethod: normalizedOldPayments[0].paymentMethod
    } : 'No old payments');
    ...
    console.log('📊 Admin: Normalized Razorpay payments sample:', normalizedRazorpayPayments[0] ? {
      id: normalizedRazorpayPayments[0]._id,
      source: normalizedRazorpayPayments[0].source,
      paymentMethod: normalizedRazorpayPayments[0].paymentMethod
    ...
    console.log('📊 Admin: Total combined payments:', allPayments.length);
    console.log('📊 Admin: Normalized old payments count:', normalizedOldPayments.length);
    console.log('📊 Admin: Normalized Razorpay payments count:', normalizedRazorpayPayments.length);
    
    // Debug: Log sample payments to see source field
    if (allPayments.length > 0) {
      console.log('📊 Admin: Sample payment sources:', allPayments.slice(0, 3).map(p => ({
        id: p._id,
        source: p.source,
