    ...
    <header className="bg-white shadow-sm border-b border-gray-200 fixed top-0 left-0 right-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Principal Panel Label */}
          <div className="hidden lg:flex items-center space-x-4">
            <span className="text-green-600 font-semibold">Principal Portal</span>
            {user ? (
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2 text-gray-700">
                  <User size={18} />
                  <span className="text-sm font-medium">
                    {user.principalName || user.email} (principal)
                  </span>
                </div>
                <button
                  onClick={logout}
          ...
          <div className="lg:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 border-t border-gray-200">
              <div className="pt-4 space-y-2">
                <div className="text-green-600 font-semibold px-3 py-2">Principal Portal</div>
                {user ? (
                  <>
                    <div className="flex items-center space-x-2 text-gray-700 px-3 py-2">
                      <User size={18} />
                      <span>{user.principalName || user.email} (principal)</span>
                    </div>
                    <button
                      onClick={() => {
                        logout();
